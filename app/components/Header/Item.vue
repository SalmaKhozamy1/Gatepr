    ...
    <template v-if="name">
        ...
        <div v-if="isDropdownOpen" class="profile-dropdown">
            <div class="dropdown-header">
                <h5 class="user-full-name mb-1">{{ name }} عبد الله</h5>
                <h6 class="desc">أدمن</h6>
            </div>
            <ul class="dropdown-list custom-ul">
                <li class="dropdown-item" @click.stop="openChangePassword">
                    <IconsLock />
                    <h5 class="item-text">تعديل كلمة المرور</h5>
                </li>
                <li class="dropdown-item logout" @click="handleLogout">
                    ...
                    <h5 class="item-text">تسجيل الخروج</h5>
                </li>
            </ul>
        </div>
    </template>
